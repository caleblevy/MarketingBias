...
def f_stat(predicate_dir, data):
    ...
    for m in user_attrs:
        for n in model_attrs:
            ...
            U += u_market_segment
    f1, p = sm.stats.anova_lm(ols('error ~ model_attr*user_attr - model_attr - user_attr', data=data).fit()).values[0, -2:]
    f2 = (V / (M * N - 1)) / (U / (data.shape[0] - (M * N)))
    print(f2, f1, p)
    return f1
...
segment_avg = pd.read_csv("results/modcloth/user_parity_fairness/baseline_split/inferred_predicates/TARGETSEGMENTRATINGAVG.txt", sep='\t', names=["user_attr", "model_attr", "rating_average"])
...
# print(modcloth.query("split==2")[["user_id", "item_id", "user_attr", "model_attr", "rating"]].dropna()
#               .groupby(["user_attr", "model_attr"])["rating"].mean().reset_index())
...
train = modcloth.query("(split == 0) | (split == 1)")
pandas_obs_item = train.groupby(["user_attr", "model_attr", "item_id"])["rating"].mean().reset_index().groupby(["user_attr", "model_attr"])["rating"].mean().reset_index()
psl_obs_item = pd.read_csv("datasets/modcloth/predicates/baseline_split/eval/observations/ObsSegmentItemAvg.txt", sep='\t', names=["user_attr", "model_attr", "rating"])
print(pandas_obs_item)
print(psl_obs_item)

pandas_obs_user= train.groupby(["user_attr", "model_attr", "user_id"])["rating"].mean().reset_index().groupby(["user_attr", "model_attr"])["rating"].mean().reset_index()
psl_obs_user = pd.read_csv("datasets/modcloth/predicates/baseline_split/eval/observations/ObsSegmentUserAvg.txt", sep='\t', names=["user_attr", "model_attr", "rating"])
print(pandas_obs_user)
print(psl_obs_user)

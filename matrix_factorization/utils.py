...
class Model(object):
    ...
    def evaluate_rating(self, df_validation, df_test, filename=None):
        ...
        with tf.Graph().as_default(), tf.Session() as session:
            ...
            if(filename != None):
                df_validation.to_csv(filename + '_rating_validation.csv', header = True, index = False)
                df_test.to_csv(filename + '_rating_test.csv', header = True, index = False)
    ...
    def evaluate_ranking(self, df_validation, df_test, filename=None, topK=10):
        ...
        with tf.Graph().as_default(), tf.Session() as session:
            ...
            for _vars in batches_vali:
                ...
                print('scores_batch')
                print('score shape', np.array(_scores_batch).shape)
                print('vars shape', _vars[:, 0].shape)
            ...
            for _ai in range(len(item_attr_ids)):
                df_validation[item_attr_ids[_ai]] = _recommended_distribution[:, _ai]
            df_recommend = df_validation.groupby('user_attr')[item_attr_ids].sum()
            df_count = df_validation.groupby(['user_attr', 'model_attr'])['rating'].size().unstack()

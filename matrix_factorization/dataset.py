...
class Dataset(object):
    ...
    def __init__(self, DATA_NAME):
        ...
        data.to_csv("modcloth.txt", header = True, index = False, sep = '\t', line_terminator = '\n')
    ...
    def get_user_item_train_map(self):
        ...
        d = (self.data.loc[(self.data['rating']>=4) & (self.data['split'] == 0)]).groupby(['user_id'])['item_id'].apply(list)
        user_item_train_map = (self.data.loc[(self.data['rating']>=4) & (self.data['split'] == 0)]).groupby(['user_id'])['item_id'].apply(list).to_dict()
